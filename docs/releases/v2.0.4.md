...
- Scope: first-class Docker deployment support, stale working-agent recovery, official direct-API preset onboarding, Kimi Code provider support, and development-pack-safe API assignment.
...
### 5. API Model Assignment Now Respects the Development Pack Scope

- When assigning API-backed models to agents, the assign flow now loads only `development` workflow-pack departments.
- Result: the API assignment modal no longer mixes unrelated pack departments into the default development assignment workflow.
