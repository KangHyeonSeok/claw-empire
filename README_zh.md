...
<p align="center">
  ...
  一个本地优先的 AI 代理办公室模拟器，可将 <b>CLI</b>、<b>OAuth</b>、<b>API 连接</b> 的提供商（如 <b>Claude Code</b>、<b>Codex CLI</b>、<b>Gemini CLI</b>、<b>OpenCode</b>、<b>Kimi Code</b>、<b>GitHub Copilot</b>、<b>Antigravity</b>）统一编排为自治虚拟公司
</p>
...
- **Docker 部署现已成为正式支持路径** - 仓库现已提供面向生产的 `Dockerfile`、`docker-compose.yml`、`.dockerignore` 以及面向容器环境的运行时路径处理。
- **中断执行后遗留的 `working` 代理会自动恢复** - lifecycle 启动与周期扫描现在会把没有活跃 `in_progress` 任务的 `working` 代理重置为 `idle`。
- **设置 > API 现在支持官方 direct API 预设** - OpenCode Go 与 Bailian Coding Plan 预设会锁定正确的 Base URL、立即注入 fallback 模型，并使用显式的 refresh/retry 流程。
- **Kimi Code 已实现端到端接入** - Kimi 已接入 CLI 执行、提供商标签、技能 learn/unlearn、prompt skills、video-preprod bootstrap，以及 provider 相关的 schema/runtime 检查。
- **API 模型分配现在会保持在 development pack 范围内** - 默认开发分配流程不会再混入其他 office pack 的部门。
- **本地 E2E 验证现在更安全也更干净** - `pnpm run test:e2e` 会在 Playwright 运行前后重置隔离运行时，已有的 `8810` 本地服务器也只会在显式 opt-in 时才复用。

- 详细说明: [`docs/releases/v2.0.4.md`](docs/releases/v2.0.4.md)
...
**多提供商 CLI** — 配置 Claude Code、Codex、Gemini CLI、OpenCode、Kimi Code 并选择模型
...
| **多提供商支持**      | Claude Code、Codex CLI、Gemini CLI、OpenCode、Kimi Code、Antigravity — 统一仪表板管理                                                |
| **外部 API 提供商**   | 通过设置 > API 选项卡将代理连接到外部 LLM API（OpenAI、Anthropic、Google、Ollama、OpenRouter、Together、Groq、Cerebras、自定义端点），并可直接使用 OpenCode Go / Bailian 官方预设 |
...
说明：`pnpm run test:e2e` 现在会启动隔离的 `.tmp/e2e-runtime` 数据库，并在执行结束后再次清理。只有在你明确想复用已运行的本地 `8810` 服务时，才设置 `PW_REUSE_EXISTING_SERVER=1`。
...
现在可在 **设置 > API** 选项卡中直接添加以下 4 个官方 direct API 预设：
...
- Bailian Coding Plan API Key 必须以 `sk-sp-` 开头。
- 这些预设会在保存后立即写入 fallback 模型列表，因此即使 live `/models` 发现失败或不完整，也能马上分配给代理。
- direct API 预设使用 `glm-5`、`kimi-k2.5`、`minimax-m2.5` 这类真实 endpoint model id，不使用 `opencode-go/<model-id>` 这类 OpenCode CLI 模型 ID。
- Bailian Coding Plan 密钥是面向交互式编码工具流程说明的密钥，在其他环境复用前请先阅读官方文档。
