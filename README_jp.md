...
<p align="center">
  ...
  <b>CLI</b>、<b>OAuth</b>、<b>API連携</b>プロバイダー（<b>Claude Code</b>、<b>Codex CLI</b>、<b>Gemini CLI</b>、<b>OpenCode</b>、<b>Kimi Code</b>、<b>GitHub Copilot</b>、<b>Antigravity</b> など）を自律エージェントの仮想企業として統合運用するローカルファーストAIエージェントオフィスシミュレーター
</p>
...
<p align="center">
  ...
  <a href="docs/releases/v2.0.4.md">リリースノート</a> &middot;
  ...
  <a href="#dockerデプロイクイック">Dockerデプロイ</a> &middot;
...
- **Dockerデプロイが正式な運用パスとして追加されました** - プロダクション向け `Dockerfile`、`docker-compose.yml`、`.dockerignore`、コンテナ環境向けランタイムパス整理が含まれます。
- **中断後に `working` のまま残ったエージェントを自動復旧します** - lifecycle の起動時/定期スイープが、生きた `in_progress` タスクを持たない `working` エージェントを `idle` に戻します。
- **Settings > API で公式 direct API プリセットが使えるようになりました** - OpenCode Go と Bailian Coding Plan のプリセットが Base URL 固定、fallback モデル初期化、明示的な refresh/retry 動線を提供します。
- **Kimi Code が end-to-end でサポートされました** - CLI 実行、プロバイダー表示、skills learn/unlearn、prompt skills、video-preprod bootstrap、既存 schema/runtime の provider チェックまで接続されています。
- **API モデル割り当ては development パックのスコープを守ります** - 既定の開発向け割り当てフローに他の office pack の部署が混ざりません。
- **ローカル E2E 検証がより安全でクリーンになりました** - `pnpm run test:e2e` は Playwright 実行の前後で隔離ランタイムを初期化し、既存の `8810` サーバー再利用も明示的に opt-in した場合のみ有効です。

- 詳細: [`docs/releases/v2.0.4.md`](docs/releases/v2.0.4.md)
...
**マルチプロバイダーCLI** — Claude Code、Codex、Gemini CLI、OpenCode、Kimi Code をモデル選択付きで設定
...
| **マルチプロバイダー対応**      | Claude Code、Codex CLI、Gemini CLI、OpenCode、Kimi Code、Antigravity — すべて一つのダッシュボードから管理                                    |
| **外部APIプロバイダー**         | 設定 > APIタブからエージェントを外部LLM API（OpenAI、Anthropic、Google、Ollama、OpenRouter、Together、Groq、Cerebras、カスタム）に接続し、OpenCode Go / Bailian の公式プリセットも利用可能 |
...
このリポジトリは本番運用向けの Docker 既定設定を同梱しています。

- **非rootユーザー**（`app`, uid/gid `10001`）で実行
- 必須ツール（`git` / `bash` / `openssh-client`）を同梱
- 標準ファイル名 `docker-compose.yml` + `Dockerfile` を使用
- 実行データは `./data` に永続化（gitignore 済み）
...
機密値は `.env.docker.private` に保存（ローカルのみ）：

```bash
cat > .env.docker.private <<'EOF'
# Claude Code 互換エンドポイント
...
> `.env.docker*` は `.env.*` ルールで git 管理対象外です。
...
アクセス: `http://127.0.0.1:8790`

### 任意: GHCR へイメージ公開

```bash
# packages write 権限付き GitHub Token が必要
...
注: `pnpm run test:e2e` は隔離された `.tmp/e2e-runtime` DB を起動し、実行後に再度クリアします。すでに起動しているローカル `8810` サーバーを意図的に再利用したい場合だけ `PW_REUSE_EXISTING_SERVER=1` を設定してください。
...
**Settings > API** タブでは、次の4つの公式 direct API プリセットをすぐに追加できます。
...
- Bailian Coding Plan の API キーは `sk-sp-` プレフィックスで始まる必要があります。
- これらのプリセットは保存直後に fallback モデル一覧をシードするため、live `/models` 検出が不完全でもすぐに割り当てできます。
- direct API プリセットは `glm-5`、`kimi-k2.5`、`minimax-m2.5` のような実際の endpoint model id を使います。`opencode-go/<model-id>` のような OpenCode CLI モデル ID は使いません。
- Bailian Coding Plan のキーは対話型コーディングツール向けに案内されるものです。他用途へ流用する前に公式ドキュメントを確認してください。
